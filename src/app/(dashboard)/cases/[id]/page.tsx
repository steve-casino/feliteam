...
const CaseDetailPage: React.FC = () => {
  ...
  const [isUrgent, setIsUrgent] = useState(caseObj?.is_urgent || false)
  ...
  const handleAddNote = () => {
    if (newNoteContent.trim()) {
      // In real app, would submit to DB
      setNewNoteContent('')
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <Button variant="primary" onClick={() => setIsStageChangeOpen(false)}>
              Update Stage
            </Button>
            <Button
              variant="secondary"
                ...
                return (
                  <div key={note.id} className="p-4 rounded-lg bg-navy-200/30 border border-white/5">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <span className="px-2 py-1 text-xs rounded bg-blue-600/20 text-blue-300">
                        {note.type === 'call_log'
                          ? 'Call'
